...
public interface UserMapper {
    ...
    User getUserByUserid(@Param("userid") Integer userid);//根据用户userid获得对应用户信息
    ...
    //用户更新头像
    @Update("UPDATE user SET picurl = #{picurl} WHERE userid = #{userid}")
    int updatePicurlByUserid(@Param("userid") Integer userid, @Param("picurl") String picurl);

    int update(User user);//已经在对应的User.xml映射文件中定义了相应的SQL更新语句

    @Delete("delete from user where userid = #{userid}")
    Integer deleteByUserid(@Param("userid") Integer userid);//使用@Delete注解定义了一个SQL删除操作，根据提供的用户userid删除use表中的记录
